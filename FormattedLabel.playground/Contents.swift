...
class TagParser: NSObject {
    ...
    class func parse(tag: Tag, originString: String, match: NSTextCheckingResult) -> Item? {
        ...
        default:
            if let endRegex = try? NSRegularExpression(pattern: ">", options: []),
    ...
    class func parse(originString: String) -> String {
        ...
        textItems.compactMap({ $0.value }).forEach { value in
            valueString += value
        }
...
let text = "dhvjkd <text=\"Anton\">"

let parsedText = TagParser.parse(originString: text)
